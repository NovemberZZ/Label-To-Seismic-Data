...
print('Loading data ...')
input_data_3d = segy.readsegy(filename_in='./test_data/test_data_kerry_filter_3d.segy', inline_byte=9, xline_byte=13)
input_data_3d = np.transpose(input_data_3d, [2, 1, 0])  # Kerry
...
model.load_state_dict(
    torch.load('./model/label_2_kerry_no_seed_epoch_48/checkpoint_depth5_wf5_bs100_lr1e_5_epoch200.pkl'))

kernel = win_func.overlap_kernel(windows_size=win_size, overlap=overlap)

output_data_3d = np.zeros(np.shape(input_data_3d))
print('Recognition ...')
for inline_num in range(np.size(input_data_3d, 0)):
    print(inline_num)

    input_data = input_data_3d[inline_num, :, :]
    ...
    # output_data = np.flip(output_data, 1)  # ****************** #
    output_data_3d[inline_num, :, :] = output_data

output_data_3d = np.transpose(output_data_3d, [2, 1, 0])  # Kerry
# print('Writing SEGY ...')
# segy.writesegy(filename_in='./test_data/test_data_kerry_filter_3d.segy', inline_byte=9, xline_byte=13,
#                data=output_data_3d, filename_out='./test_result/test_data_kerry_filter_3d_out.segy')
...
output_data_3d2 = np.zeros(np.shape(input_data_3d))
print('Recognition ...')
for inline_num in range(np.size(input_data_3d, 0)):
    print(inline_num)
    ...
    output_data = output_data[overlap:np.size(output_data, 0)-win_size, overlap:np.size(output_data, 1)-win_size]
    # output_data = np.flip(output_data, 1)  # ****************** #
    output_data_3d2[inline_num, :, :] = output_data

# output_data_3d = np.transpose(output_data_3d, [2, 1, 0])  # Kerry
output_data_3d2 = output_data_3d2 + output_data_3d
print('Writing SEGY ...')
segy.writesegy(filename_in='./test_data/test_data_kerry_filter_3d.segy', inline_byte=9, xline_byte=13,
               data=output_data_3d2, filename_out='./test_result/test_data_kerry_filter_3d_out_merge.segy')
